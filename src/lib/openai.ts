...
export async function generateMidjourneyPrompt(
  ...
  guidance?: string
): Promise<{ prompt: string; midjourneyPrompts: string[]; outfitNames?: string[] }> {
  try {
    console.log(
...
${promptType === "outfit" ? `Create 3 prompts focusing on the ${clothingPart} worn by a ${genderType} model.
Requirements:
...
Do not include /imagine command.`,
        },
        {
          ...
          content: [
            {
              type: "text",
              text: `Analyze this image and create ${promptType} prompts for ${clothingPart}.${guidance ? ` User guidance: ${guidance}` : ""}`,
            },
            {
              type: "image_url",
              image_url: {
